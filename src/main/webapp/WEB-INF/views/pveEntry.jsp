...
<main class="msl-main">
    ...
    <header class="msl-page-header msl-animate">
        <div class="msl-page-header-left">
            ...
            <div class="msl-page-sub">PVE 스테이지에 출전할 7명의 선수를 선택하세요.</div>
        </div>
    </header>
                            ...
                            <c:otherwise>
                                <c:forEach var="player" items="${allOwnedPlayers}">
                                    <tr class="myteam-row"
                                        ...
                                        data-mac="${player.currentMacro   + player.enhanceMacro}"
                                        data-mic="${player.currentMicro   + player.enhanceMicro}"
                                        data-lck="${player.currentLuck    + player.enhanceLuck}">
                                        <td class="td-chk">
        ...
        <div class="msl-panel analysis-panel">
            ...
            <div class="msl-panel-body analysis-body">
                ...
                <div class="ana-section ana-slots-section">
                    <div class="ana-title">선발 진행 <span id="slotLabel" style="float:right;font-size:0.78rem;letter-spacing:0;color:var(--text-dim);font-family:inherit;">0 / 7</span></div>
...
<script>
    ...
    function refreshAnalysis() {
        ...
        document.getElementById('slotLabel').textContent = n + ' / 7';
        ...
        var statMap = [
            ...
            {n:'매크로', v:aMac, c:'#34d399'},
            {n:'컨트롤', v:aMic, c:'#fbbf24'},
            {n:'운', v:aLck, c:'#a78bfa'}
        ];
        ...
        if (n < 7) {
            msgs.push({type:'action', icon:'📋', text:'<strong>' + (7-n) + '명</strong>을 더 선택해야 합니다.'});
        } else {
            msgs.push({type:'complete', icon:'✅', text:'7명 선발 완료! <strong style="color:#00e676">저장</strong>할 수 있습니다.'});
        }
    ...
    function drawRadar(aAtk, aDef, aMac, aMic, aLck) {
        ...
        var labels = ['ATK','DEF','MAC','MIC','LCK'];
        ...
        vals.forEach(function(v, i) {
            var dp  = pt(i, v);
            var lp  = axis(i, 1.22);  // 레이블 위치

            // 수치를 데이터 포인트에서 약간 바깥쪽으로 오프셋
            var angle = (Math.PI * 2 / n) * i - Math.PI / 2;
            var vr    = Math.min(Math.max(v / MAX, 0.04), 1);
            var offR  = vr * r + 13;  // 점에서 13px 바깥
    ...
    function saveEntry() {
        var checked = document.querySelectorAll('.entry-chk:checked');
        if (checked.length !== 7) {
            alert('7명을 모두 선택해주세요. (현재 ' + checked.length + '명)');
            return;
        }
        var seqList = [];
